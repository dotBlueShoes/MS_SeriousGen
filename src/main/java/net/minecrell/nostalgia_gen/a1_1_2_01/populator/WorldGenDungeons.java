      ...
      for(x = baseX - var7 - 1; x <= baseX + var7 + 1; ++x) {
         for(y = baseY - 1; y <= baseY + var6 + 1; ++y) {
            for(x = baseZ - var8 - 1; x <= baseZ + var8 + 1; ++x) {
               ...
               if ((x == baseX - var7 - 1 || x == baseX + var7 + 1 || x == baseZ - var8 - 1 || x == baseZ + var8 + 1) && y == baseY && NostalgiaGenHelper.getBlock(world, x, y, x) == Blocks.AIR && NostalgiaGenHelper.getBlock(world, x, y + 1, x) == Blocks.AIR) {
                  ++var9;
               }
            }
         }
      ...
      if (var9 >= 1 && var9 <= 5) {
         ...
         for(x = 0; x < 2; ++x) {
            for(y = 0; y < 3; ++y) {
               ...
               if (NostalgiaGenHelper.getBlock(world, x, baseY, z) == Blocks.AIR) {
                  int var15 = 0;
